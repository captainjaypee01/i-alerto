...
$factory
    ->state(BackpackUser::class, 'barangay', [])
    ->afterCreatingState(BackpackUser::class, 'barangay', function ($user, $faker) {
        $user->assignRole('barangay');

        factory(Barangay::class)->create([
            'user_id' => $user->id,
            'first_name' => $user->first_name,
            'middle_name' => $user->middle_name,
            'last_name' => $user->last_name,
